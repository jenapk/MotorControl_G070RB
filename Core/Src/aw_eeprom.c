...
/* Application */
#include "aw_eeprom.h"             /* EEPROM MW Layer */
...
{
	uint8_t l_retEep;
	uint8_t l_buffW[2] = {0,0};
	uint8_t l_buffR[2] = {0,0};

	l_retEep = EPR_Read((EPR_DEVICE_SIZE -2), l_buffR, 2);
	...
	l_retEep = EPR_Write((EPR_DEVICE_SIZE -2), l_buffW, 2);
	l_retEep = EPR_Read((EPR_DEVICE_SIZE -2), l_buffR, 2);
	return(l_buffR[0] == l_buffW[0]);
}